var Snake = function (direction, body) {
	'use strict';
	this.body = body || [ { x: 10, y: 5}, { x: 10, y: 6}, { x: 10, y: 7}, { x: 10, y: 8} ];
	...
	this.checkIntersection = function () {
		var i;
		for (i = this.body.length - 1; i > 0; i -= 1) {
...
var Level = function (snake, width, height, increment) {
	'use strict';
...
var Game = function (canvas, level, score, time) {
	...
	this.loop = function () {
		...
		if ((that.level.food[0].x === that.level.snake.body[0].x) && (that.level.food[0].y === that.level.snake.body[0].y)) {
			that.level.food.shift();
			that.level.snake.grow();
			that.level.createFood(Math.floor(Math.random() * (that.level.width / that.level.increment)), Math.floor(Math.random() * (that.level.height / that.level.increment)));
			that.score += 10;
		}
